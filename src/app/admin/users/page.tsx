...
const emptyForm = { name: "", userName: "", email: "", password: "", branchId: "", isActive: "Y" };
...
export default function UsersPage() {
  ...
  const openEdit = (u: AdminUser) => {
    setEditing(u);
    setForm({ name: u.name ?? "", userName: u.userName, email: u.email ?? "", password: "", branchId: String(u.branchId), isActive: u.isActive ?? "Y" });
    setModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const base = { name: form.name || undefined, email: form.email || undefined, branchId: form.branchId, isActive: form.isActive };
      if (editing) {
    ...
    <AppLayout>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Edit User" : "New User"}>
        <div className="grid grid-cols-2 gap-4">
          <Input label="Username *" value={form.userName} onChange={(e) => setForm({ ...form, userName: e.target.value })} disabled={!!editing} placeholder="john.doe" />
          <Input label="Full Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="John Doe" />
          <Input label={editing ? "Email" : "Email *"} type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} disabled={!!editing} placeholder="user@example.com" />
          <Input label={editing ? "New Password (leave blank to keep)" : "Password *"} type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder={editing ? "Leave blank to keep current" : "Min 6 characters"} />
          <Select label="Branch *" value={form.branchId} onChange={(e) => setForm({ ...form, branchId: e.target.value })} placeholder="Select branch..." options={branches.map((b) => ({ value: b.id, label: b.branchName }))} />
        ...
        {!editing && <p className="text-xs text-gray-400 mt-3">A verification email will be sent to the address above after the user is created.</p>}
